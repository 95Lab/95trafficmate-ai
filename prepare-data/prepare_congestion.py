...
datapath = "../congestion_{}_60.csv"

chunksize = 1e7
dfs = []
cnt = 0
datapath_ = datapath.format(year)
chunks = pd.read_csv(datapath_, chunksize=chunksize, usecols=['start_datetime', 'meshcode', 'avg_length'])
for chunk in chunks:
    cnt += 1
...
df.to_csv(f'{project_data_path}/dataframe/congestion_{year}.csv', index=False)