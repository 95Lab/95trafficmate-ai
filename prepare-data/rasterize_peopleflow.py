...
df = pd.read_csv(f'{project_data_path}/raster/peopleflow_{year}.csv')

if year == 2020:
    mval, maxval = compute_minmaxval(df, 'avg_floating_population')
    with open(f'{project_data_path}/raster/peopleflow/maval.txt', 'w') as f:
        f.write(f'{maxval:.2f}')
else:
    with open(f'{project_data_path}/raster/peopleflow/maval.txt', 'r') as f:
        maxval = float(f.read())

dfx = pixelize_dataframe(df, 'avg_floating_population', maxval)
start_time = f"{year}-01-01 00:00:00+09"
end_time = f"{year}-06-30 23:00:00+09"
full_time = make_full_time(start_time, end_time)
output_data_path = f'{project_data_path}/raster/peopleflow/{year}'
